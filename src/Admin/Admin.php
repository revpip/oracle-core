...
final class Admin
{
    public function register(): void
    {
        ...
        add_action('admin_post_oracle_import_questions', [$this, 'importQuestions']);
    }

    public function menu(): void
    {
        add_menu_page(__('Oracle Core', 'oracle-core'), __('Oracle Core', 'oracle-core'), 'manage_options', 'oracle-core', [$this, 'dashboard'], 'dashicons-chart-area', 58);
    ...
    public function dashboard(): void
    {
        ...
        <div class="wrap oracle-admin">
            ...
            <div class="oracle-panel"><h2>Launch shortcode</h2><code>[oracle_assessment]</code><p>Add this shortcode to any WordPress page to launch the first assessment.</p></div>
            <div class="oracle-panel"><h2>Current build</h2><p><strong>Version:</strong> <?php echo esc_html(ORACLE_CORE_VERSION); ?></p><p><strong>Status:</strong> Sprint 1 question bank management. Safe self-reflection wording only. No diagnostic claims.</p></div>
        </div>
        <?php
    }
    ...
    public function questionsPage(): void
    {
        ...
        <div class="wrap oracle-admin">
            ...
            <?php if (isset($_GET['imported'])): ?><div class="notice notice-success"><p>Imported <?php echo esc_html(absint($_GET['imported'])); ?> question(s).</p></div><?php endif; ?>
            <?php if (isset($_GET['saved'])): ?><div class="notice notice-success"><p>Question saved.</p></div><?php endif; ?>
            ...
            <div class="oracle-panel">
                <h2>Import / export</h2>
                <p>CSV headers: <code>category,dimension,question_text,help_text,weight,sort_order,is_active</code></p>
                <form method="post" enctype="multipart/form-data" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                    <input type="hidden" name="action" value="oracle_import_questions">
                    <?php wp_nonce_field('oracle_import_questions', 'oracle_import_nonce'); ?>
                    <input type="file" name="oracle_questions_csv" accept=".csv,text/csv" required>
                    <?php submit_button('Import CSV', 'secondary', 'submit', false); ?>
                    <a class="button" href="<?php echo esc_url(wp_nonce_url(admin_url('admin-post.php?action=oracle_export_questions'), 'oracle_export_questions')); ?>">Export CSV</a>
                </form>
            </div>
                ...
                <table class="widefat striped oracle-table">
                    ...
                    <?php foreach ($questions as $q): ?>
                        <tr>
                            ...
                            <td><a class="button button-small" href="<?php echo esc_url(admin_url('admin.php?page=oracle-core-questions&edit=' . absint($q->id))); ?>">Edit</a> <a class="button button-small" href="<?php echo esc_url(wp_nonce_url(admin_url('admin-post.php?action=oracle_toggle_question&id=' . absint($q->id)), 'oracle_toggle_question')); ?>"><?php echo $q->is_active ? 'Deactivate' : 'Activate'; ?></a></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
    ...
    {
        if (!current_user_can('manage_options') || !isset($_POST['oracle_import_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['oracle_import_nonce'])), 'oracle_import_questions')) {
            wp_die('Security check failed.');
        }
        if (empty($_FILES['oracle_questions_csv']['tmp_name']) || !is_uploaded_file($_FILES['oracle_questions_csv']['tmp_name'])) {
            wp_safe_redirect(admin_url('admin.php?page=oracle-core-questions&imported=0'));
            exit;
        }
        global $wpdb;
        $table = $wpdb->prefix . 'oracle_questions';
        $handle = fopen($_FILES['oracle_questions_csv']['tmp_name'], 'r');
        if (!$handle) {
            wp_die('Could not read import file.');
        }
        $headers = fgetcsv($handle);
        $headers = is_array($headers) ? array_map('sanitize_key', $headers) : [];
        $imported = 0;
        while (($row = fgetcsv($handle)) !== false) {
            $data = array_combine($headers, array_pad($row, count($headers), ''));
            if (!$data || empty($data['question_text'])) {
                continue;
            }
            $wpdb->insert($table, [
                'uuid' => !empty($data['uuid']) ? sanitize_text_field($data['uuid']) : wp_generate_uuid4(),
                'category' => sanitize_text_field($data['category'] ?? 'General'),
                'dimension' => sanitize_key($data['dimension'] ?? 'general'),
        ...
        fclose($handle);
        wp_safe_redirect(admin_url('admin.php?page=oracle-core-questions&imported=' . absint($imported)));
        exit;
    }
    ...
    public function settingsPage(): void
    {
        ...
        <div class="wrap oracle-admin">
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table" role="presentation">
                    <tr><th scope="row"><label for="oracle_core_openai_api_key">OpenAI API Key</label></th><td><input type="password" class="regular-text" id="oracle_core_openai_api_key" name="oracle_core_openai_api_key" value="<?php echo esc_attr(get_option('oracle_core_openai_api_key', '')); ?>" autocomplete="off"><p class="description">Stored for the future AI Reflection Engine. Not used yet.</p></td></tr>
                    <tr><th scope="row"><label for="oracle_core_premium_url">Premium Report URL</label></th><td><input type="url" class="regular-text" id="oracle_core_premium_url" name="oracle_core_premium_url" value="<?php echo esc_attr(get_option('oracle_core_premium_url', '')); ?>"><p class="description">Optional button shown after results.</p></td></tr>
                    <tr><th scope="row"><label for="oracle_core_disclaimer">Disclaimer</label></th><td><textarea class="large-text" rows="4" id="oracle_core_disclaimer" name="oracle_core_disclaimer"><?php echo esc_textarea(get_option('oracle_core_disclaimer', $default_disclaimer)); ?></textarea></td></tr>
                </table>
                <?php submit_button(); ?>
            </form>
        </div>
